...
class AgentsDB:
    ...
    def create_agent(self, agent_id: str, agent_data: dict[str, Any]) -> dict[str, Any]:
        ...
        agent = {
            ...
            "enable_self_correction": agent_data.get("enable_self_correction", False),
            "version": 1,
            "created_at": now,
            "updated_at": now,
            "archived_at": None,
    ...
    def update_agent(
        ...
        if "metadata" in update_data and update_data["metadata"] is not None:
            new_agent["metadata"] = {
                **current_agent.get("metadata", {}),
                **update_data["metadata"],
            }
            # Set to empty string to delete a key (as per spec)
            new_agent["metadata"] = {
                k: v for k, v in new_agent["metadata"].items() if v != ""
            }
        ...
        self._validate_agent(new_agent)
    ...
    def _is_noop(self, old: dict, new: dict) -> bool:
        ...
        return all(old.get(f) == new.get(f) for f in fields_to_check)

    def _validate_agent(self, agent: dict[str, Any]):
        """Validate agent data (e.g. tools, mcp_servers)."""
        if agent.get("tools"):
            for tool in agent["tools"]:
                if not isinstance(tool, dict) or "name" not in tool:
                    logger.warning("Agent {} has invalid tool: {}", agent["id"], tool)

        if agent.get("mcp_servers"):
            for server in agent["mcp_servers"]:
                if not isinstance(server, dict) or "url" not in server:
                    logger.warning(
                        "Agent {} has invalid MCP server: {}", agent["id"], server
                    )
